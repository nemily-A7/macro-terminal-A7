...
def show():
    page_header("Interest Rates", "Policy Rates & Yield Curves")
    
    zone, country = zone_country_selector(key_prefix="rates")
    
    indicators = get_indicators_by_category(zone, "Rates", country)
    
    if not indicators:
        st.warning(f"No rate indicators found for {zone}")
        return
