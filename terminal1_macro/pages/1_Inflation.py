...
def show():
    page_header("Inflation", "CPI, PCE, PPI Analysis")
    
    # Zone and Country selector
    zone, country = zone_country_selector(key_prefix="inflation")
    
    # Load inflation indicators for selected zone
    indicators = get_indicators_by_category(zone, "Inflation", country)
    
    if not indicators:
        st.warning(f"No inflation indicators found for {zone}")
        return
