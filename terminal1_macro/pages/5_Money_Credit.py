...
def show():
    page_header("Money & Credit", "M2, Credit Spreads")
    
    zone, country = zone_country_selector(key_prefix="money_credit")
    
    indicators = get_indicators_by_category(zone, "Money_Credit", country)
    
    if not indicators:
        st.info(f"No money/credit indicators found for {zone}")
        st.caption("Note: Money & Credit indicators are primarily available for US zone.")
        return
