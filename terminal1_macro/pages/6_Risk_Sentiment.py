...
def show():
    page_header("Risk & Sentiment", "VIX, Consumer Sentiment")
    
    zone, country = zone_country_selector(key_prefix="risk_sentiment")
    
    indicators = get_indicators_by_category(zone, "Risk_Sentiment", country)
    
    if not indicators:
        st.info(f"No risk/sentiment indicators found for {zone}")
        st.caption("Note: Risk & Sentiment indicators are primarily available for US zone.")
        return
