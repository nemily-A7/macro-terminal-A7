...
def show():
    page_header("Economic Growth", "GDP & Activity Indicators")
    
    zone, country = zone_country_selector(key_prefix="growth")
    
    indicators = get_indicators_by_category(zone, "Growth", country)
    
    if not indicators:
        st.warning(f"No growth indicators found for {zone}")
        return
